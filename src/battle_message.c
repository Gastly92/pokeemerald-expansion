...
const u8 *const gBattleStringsTable[STRINGID_COUNT] =
{
    ...
    // FORK: fork-owned entries stay at the end of this designated-initializer table (index
    // order is irrelevant here) so they never sit between upstream entries and conflict when
    // upstream edits an adjacent line. Matches the fork block at the end of battle_string_ids.h.
    [STRINGID_HARVESTHPGAIN]                        = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} restored a little HP!"), // FORK: DETERMINISTIC_ABILITIES, Harvest sun heal
...
// FORK: action menu label when the bag is replaced by the read-only INFO viewer
// in Frontier facilities (B_FRONTIER_BATTLE_INFO). See src/frontier_battle_info.c.
const u8 gText_BattleMenuInfo[] = _("Battle{CLEAR_TO 56}Info\nPokémon{CLEAR_TO 56}Run");
