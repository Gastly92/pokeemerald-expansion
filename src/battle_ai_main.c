...
static s32 (*const sBattleAiFuncTable[])(enum BattlerId, enum BattlerId, enum Move, s32) =
{
    ...
    [57] = NULL,                     // Unused
    [58] = NULL,                     // Unused
    [59] = AI_GetSpeciesOverrideMoveScore, // FORK: AI_FLAG_SMART_SPECIES_LOGIC
...
static s32 AI_CheckBadMove(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, s32 score)
{
    ...
    if (moveTarget != TARGET_USER)
    {
        ...
        if (hasTwoOpponents && CanMoveBeBouncedBack(battlerAtk, move)
         && aiData->abilities[GetPartnerBattler(battlerDef)] != ABILITY_MAGIC_BOUNCE
         && IsInnateActive(GetPartnerBattler(battlerDef), ABILITY_MAGIC_BOUNCE))
            RETURN_SCORE_MINUS(20);
...
// the following checks apply to any target (including user)
    ...
    case EFFECT_SHED_TAIL:
        ...
        break;
    case EFFECT_LEECH_SEED:
        // FORK: split out of upstream's single `if` so BUFF_LEECH_SEED can re-score the
        // already-seeded / partner-duplicate cases below. On conflict, port upstream's added
        // conditions into the matching branch here rather than collapsing this back.
    ...
    case EFFECT_SLEEP_TALK:
        ...
        if (IsWakeupTurn(battlerAtk)
         || (!IsAsleepOrComatose(battlerAtk, aiData->abilities[battlerAtk]) && !BattlerHasAbility(battlerAtk, ABILITY_COMATOSE)))
...
static bool32 DoesAbilityBenefitFromSunOrRain(enum BattlerId battler, enum Ability ability, u32 weather)
{
    // FORK: credit an active innate Swift Swim / Chlorophyll / Leaf Guard (FEATURE_INNATE_ABILITIES)
    // so the AI's weather-move scoring values setting rain/sun for an innate speed-doubler or sun
    // status-immunity.
...
static s32 AI_CalcMoveEffectScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    if (gBattleMoveEffects[moveEffect].encourageEncore
     ...
     && (B_MENTAL_HERB < GEN_5 || aiData->holdEffects[battlerAtk] != HOLD_EFFECT_MENTAL_HERB))
     {
        ...
        if ((!AI_IsAbilityOnSide(battlerAtk, ABILITY_AROMA_VEIL)
          && !AI_IsInnateOnSide(battlerAtk, ABILITY_AROMA_VEIL)) // FORK: innate Aroma Veil protects the AI's side from Encore too (Batch U)
    ...
    if (gAiThinkingStruct->aiFlags[battlerAtk] & AI_FLAG_SMART_SWITCHING
        && (aiData->abilities[battlerAtk] == ABILITY_NATURAL_CURE
            || BattlerHasAbility(battlerAtk, ABILITY_NATURAL_CURE))) // FORK: innate-aware (FEATURE_INNATE_ABILITIES)
    {
    ...
    case EFFECT_FORESIGHT:
        if (aiData->abilities[battlerAtk] == ABILITY_SCRAPPY || aiData->abilities[battlerAtk] == ABILITY_MINDS_EYE
         || IsInnateActive(battlerAtk, ABILITY_SCRAPPY)
         || IsInnateActive(battlerAtk, ABILITY_MINDS_EYE)) // FORK: an innate Scrappy / Mind's Eye already hits Ghosts (and ignores evasion), so Foresight is pointless
            break;
