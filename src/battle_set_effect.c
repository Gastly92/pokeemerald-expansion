...
static void HandleSetEffectFlinch(struct BattleCalcValues *cv, struct SetEffect *se)
{
    ...
    if (cv->abilities[se->effectBattler] == ABILITY_INNER_FOCUS || IsInnateActive(se->effectBattler, ABILITY_INNER_FOCUS))
    {
...
static void HandleSetEffectRandomFromList(struct BattleCalcValues *cv, struct SetEffect *se)
{
    ...
    // FORK: DETERMINISTIC_MOVE_RESULTS removes the roll from the two moves that use this
    // mechanism, keeping the per-move heuristics the flag shipped with:
    //   Tri Attack - hit the target's stronger offense (burn if Atk > SpAtk, frostbite if
    //                SpAtk > Atk, paralysis on a tie).
    //   Dire Claw  - paralyse if Speed beats either defense, sleep on a three-way tie,
    //                else poison.
    // The chosen effect is matched back to its slot rather than assumed at a fixed index,
    // because the order of argument.randomMoveEffects is upstream's to change.
    if (GetConfig(DETERMINISTIC_MOVE_RESULTS))
    {
        struct BattlePokemon *target = &gBattleMons[se->effectBattler];
        enum MoveEffect wanted = sRandomFromListEffects[0];

        if (gCurrentMove == MOVE_TRI_ATTACK)
        {
            if (target->attack > target->spAttack)
                wanted = MOVE_EFFECT_BURN;
            else if (target->spAttack > target->attack)
                wanted = MOVE_EFFECT_FREEZE_OR_FROSTBITE;
            else
                wanted = MOVE_EFFECT_PARALYSIS;
        }
        else if (gCurrentMove == MOVE_DIRE_CLAW)
        {
            if (target->speed > target->defense || target->speed > target->spDefense)
                wanted = MOVE_EFFECT_PARALYSIS;
            else if (target->speed == target->defense && target->speed == target->spDefense)
                wanted = MOVE_EFFECT_SLEEP;
            else
                wanted = MOVE_EFFECT_POISON;
        }

        chosenMoveEffect = 0; // deterministic fallback for any future random-from-list move
        for (u32 i = 0; i < validEffectCount; i++)
        {
            if (sRandomFromListEffects[i] == wanted)
            {
                chosenMoveEffect = i;
                break;
            }
    ...
    {
        chosenMoveEffect = RandomUniform(RNG_RANDOM_FROM_LIST, 0, validEffectCount - 1);
    }
...
static void HandleSetEffectThrash(struct BattleCalcValues *cv, struct SetEffect *se)
{
    ...
    else
    {
        ...
        gBattleMons[se->effectBattler].volatiles.rampageTurns = GetConfig(DETERMINISTIC_MOVE_RESULTS)
...
static void HandleSetEffectFlameBurst(struct BattleCalcValues *cv, struct SetEffect *se)
{
    ...
    if (IsBattlerAlive(partner)
     ...
     && !IsInnateActive(partner, ABILITY_MAGIC_GUARD))
    {
...
static void HandleSetEffectIncinerate(struct BattleCalcValues *cv, struct SetEffect *se)
{
    if (cv->abilities[se->effectBattler] == ABILITY_STICKY_HOLD
     || IsInnateActive(se->effectBattler, ABILITY_STICKY_HOLD)) // FORK: innate-aware Sticky Hold (FEATURE_INNATE_ABILITIES)
        return;
...
static void HandleSetEffectPsychicNoise(struct BattleCalcValues *cv, struct SetEffect *se)
{
    ...
    if (!battler && (battler = IsInnateOnSide(se->effectBattler, ABILITY_AROMA_VEIL)))
        aromaInnate = TRUE;
...
static void HandleSetEffectLeechSeed(struct BattleCalcValues *cv, struct SetEffect *se)
{
    ...
    bool32 canSeed = buff ? !(gBattleMons[se->effectBattler].volatiles.leechSeededBy & LEECH_SEED_BIT(cv->battlerAtk))
                          : !gBattleMons[se->effectBattler].volatiles.leechSeed;

    if (!IS_BATTLER_OF_TYPE(se->effectBattler, TYPE_GRASS) && canSeed)
    {
        if (buff)
            gBattleMons[se->effectBattler].volatiles.leechSeededBy |= LEECH_SEED_BIT(cv->battlerAtk);
