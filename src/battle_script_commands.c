...
void SetNonVolatileStatus(enum BattlerId battlerAtk, enum BattlerId effectBattler, enum MoveEffect effect, const u8 *battleScript, enum StatusTrigger trigger)
{
    ...
    case MOVE_EFFECT_SLEEP:
        ...
        else if (GetConfig(B_SLEEP_TURNS) >= GEN_CHAMPIONS)
...
static void Cmd_setfocusenergy(void)
{
    ...
    {
        // NB: upstream dropped the separate laserFocus bool in the 1.17.0 sync; the timer alone is the flag now.
...
static void Cmd_recoverbasedonsunlight(void)
{
    ...
    if (gBattleMons[gBattlerAttacker].hp != gBattleMons[gBattlerAttacker].maxHP)
    {
        ...
        enum Ability ability = GetBattlerAbility(gBattlerAttacker);
        u32 attackerWeather = GetAttackerWeather(gBattlerAttacker, GetBattlerHoldEffect(gBattlerAttacker), ability, weather);
...
void BS_TryHealPulse(void)
{
    ...
    {
        s32 healAmount = 0;
        // FORK: an innate Mega Launcher boosts Heal Pulse's healing like the real ability (Batch A, FEATURE_INNATE_ABILITIES).
...
void BS_JumpIfWeatherAffected(void)
{
    ...
    if (GetAttackerWeather(gBattlerAttacker, GetBattlerHoldEffect(gBattlerAttacker), GetBattlerAbility(gBattlerAttacker), GetWeather()) & weather)
        gBattlescriptCurrInstr = cmd->jumpInstr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}
