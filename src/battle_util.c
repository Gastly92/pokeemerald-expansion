...
#include "fork/free_gimmicks.h" // FORK: FindMegaStoneForStats, the item-free Mega pick
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, enum BattlerId battler, enum Ability ability, enum Move move, bool32 shouldAbilityTrigger)
{
                 ...
                 && PickupHasValidTarget(battler))
                {
                    if (GetConfig(DETERMINISTIC_ABILITIES))
                    {
                        ...
                        enum BattlerId order[4] = {
                            ...
                            GetOppositeBattler(battler),
                            GetPartnerBattler(GetOppositeBattler(battler)),
                        };
            ...
            case ABILITY_HARVEST:
            {
                 ...
                 && GetItemPocket(GetBattlerPartyState(battler)->usedHeldItem) == POCKET_BERRIES)
                {
                    ...
                    if (GetBattlerAbility(battler) != gLastUsedAbility) // FORK: innate Harvest — pop-up shows the innate
                        gBattleScripting.abilityPopupOverwrite = gLastUsedAbility;
                    if (GetConfig(DETERMINISTIC_ABILITIES) && inSun && !IsBattlerAtMaxHp(battler) && !gBattleMons[battler].volatiles.healBlockTimer)
                    {
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_TOXIC_CHAIN:
            ...
            break;
        case ABILITY_STENCH:
            if (TryStenchFlinch()) // FORK: extracted so the innate Stench path shares it
                effect++;
            break;
        ...
        case ABILITY_MAGICIAN:
             ...
             && !gSpecialStatuses[battler].gemBoost) // In base game, gems are consumed after magician would activate.
            {
                ...
                for (u32 i = 0; i < gBattlersCount; i++)
                {
                    ...
                    if (BattlerHasAbility(targetBattler, ABILITY_STICKY_HOLD)) // FORK: innate-aware Sticky Hold (FEATURE_INNATE_ABILITIES)
                    {
                        effect = FALSE;
                        break;
                    }
...
static inline u32 CalcMoveBasePower(struct DamageContext *ctx)
{
    ...
    {
    case GIMMICK_Z_MOVE:
        // FORK: upstream returns GetZMovePower(ctx->baseMove) here. Two reasons that is not enough:
        //  1. It derives every Z-Move's power from the base move via the tier table, which is wrong
        //     for signature Z-Moves (>= MOVE_CATASTROPIKA) - they carry their own power (Genesis
        //     Supernova is 185, not whatever its base move's tier says). GetZMoveBasePower keeps
        //     that distinction and is shared with the move-selection preview, so the power the menu
        //     advertises and the power the engine deals can never drift apart.
        //  2. ctx->baseMove is only the pre-conversion move on the ordinary path (HandleAction_UseMove
        //     records it right before converting). For a called move (Metronome/Copycat/Mirror Move/
        //     Me First) the canceler converts first and assigns the *Z-Move* to gCalledMove, so the
        //     later Cmd_setcalledmove overwrites gBattleStruct->baseMove with the Z-Move. Its
        //     placeholder .power of 1 would bottom the tier table out at a flat 100 BP. zmove.baseMoves
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    // FORK: upstream inlines ctx->abilities[GetPartnerBattler(battlerAtk)] in the switch head;
    // hoisted to locals because the innate block below reuses both. On conflict, keep the locals.
    enum BattlerId partner = GetPartnerBattler(battlerAtk);
    enum Ability partnerAbility = ctx->abilities[partner]; // cache — used by the switch AND the innate block
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    // real ability. Thick Fat is a clean upside, so the innate is a 1:1 copy. The switch above already
    // applied a chosen Thick Fat, so guard against it to avoid double-applying. IsInnateActive() supplies
    // the same suppression gates (Thick Fat is breakable, so Mold Breaker pierces an innate one too).
    // Identity is untouched (no RecordAbilityBattle), matching the silent Filter/Unaware calc modifiers.
    if ((moveType == TYPE_FIRE || moveType == TYPE_ICE)
     && ctx->abilities[battlerDef] != ABILITY_THICK_FAT
     && IsInnateActive(battlerDef, ABILITY_THICK_FAT))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.5));

    // FORK: an innate Purifying Salt (FEATURE_INNATE_ABILITIES) halves incoming Ghost damage exactly like
    // the real ability (a clean upside, so a 1:1 copy). Same guards and rationale as Thick Fat above.
    if (moveType == TYPE_GHOST
...
static inline u32 CalcDefenseStat(struct DamageContext *ctx)
{
    ...
    // Defense (a physical hit), the same gate the real ability uses. Fur Coat is not breakable, so Mold
    // Breaker never touches it. AI damage prediction is correct for free (shared calc).
    ...
    // while it has any status condition, exactly like the real ability. Same guards as Fur Coat above.
    ...
    // while Grassy Terrain is up, exactly like the real ability. Same guards as Fur Coat above.
    // NB: upstream moved terrain off the gFieldStatuses bitflags onto ctx->terrain in the 1.17.0 sync.
    if (usesDefStat && ctx->terrain == B_TERRAIN_GRASSY
...
static inline uq4_12_t GetSameTypeAttackBonusModifier(struct DamageContext *ctx)
{
    if (ctx->moveType != TYPE_MYSTERY)
    {
         ...
         || (gBattleStruct->pledgeState == PLEDGE_COMBO_ATTACK && IS_BATTLER_OF_TYPE(GetPartnerBattler(ctx->battlerAtk), ctx->moveType)))
        {
            ...
            // On conflict, re-add the IsInnateActive() clause to upstream's ternary rather than dropping it.
            return (ctx->abilities[ctx->battlerAtk] == ABILITY_ADAPTABILITY
                 || (ctx->innatesEnabled && IsInnateActive(ctx->battlerAtk, ABILITY_ADAPTABILITY))) ? UQ_4_12(2.0) : UQ_4_12(1.5);
        }
    }
...
static inline uq4_12_t GetAttackerAbilitiesModifier(enum BattlerId battlerAtk, uq4_12_t typeEffectivenessModifier, bool32 isCrit, enum Ability abilityAtk, bool32 innatesEnabled)
{
    ...
    // FORK: an innate Sniper (FEATURE_INNATE_ABILITIES) boosts crit damage exactly like the real ability —
    // a clean upside, so a 1:1 copy with no pure-boon divergence. Reached only on a crit (isCrit), so the
    // GetConfig()/IsInnateActive() lookups stay off the non-crit hot path; the chosen-ability switch above
    // is untouched and abilityAtk != ABILITY_SNIPER guards against double-applying. On-field AI damage
...
static inline s32 DoMoveDamageCalc(struct DamageContext *ctx)
{
    ...
    // FORK: cache the innate-abilities flag before the crit roll so CalcCritChanceStage's innate gates read
...
s32 CalcCritChanceStage(struct DamageContext *ctx)
{
    ...
    // real ability — a clean upside, so a 1:1 copy. The innate clause is gated on the cached
    // ctx->innatesEnabled bitfield (set before the REAL crit roll by DoMoveDamageCalc /
    // DoFutureSightAttackDamageCalc), so it costs one field load and skips IsInnateActive() when off. The AI's
    // per-eval crit-chance prediction (ShouldCalcCritDamage) deliberately leaves ctx->innatesEnabled = 0, so
    // these innate reads are skipped there entirely — this calc runs per move/target in doubles and crediting
    // innate crits would blow the AI thinking-time ceiling for a negligible gain.
          ...
          // effectiveness (see AI_CalcDamage), so ctx->typeEffectivenessModifier isn't valid there yet.
          || (!ctx->aiCalc
           && GetConfig(DETERMINISTIC_CRITICAL_HITS)
    ...
    else
    {
        critChance  = (gBattleMons[ctx->battlerAtk].volatiles.focusEnergy != 0 ? 2 : 0)
                    ...
                    // ability (1:1 clean upside); the cached ctx->innatesEnabled gate skips the lookup when off.
                    + ((ctx->abilities[ctx->battlerAtk] == ABILITY_SUPER_LUCK
                     || (ctx->innatesEnabled && IsInnateActive(ctx->battlerAtk, ABILITY_SUPER_LUCK))) ? 1 : 0)
    ...
    if (!defArmor && ctx->innatesEnabled)
...
s32 CalcCritChanceStageGen1(struct DamageContext *ctx)
{
    ...
    if (ctx->abilities[ctx->battlerAtk] == ABILITY_SUPER_LUCK
     || (ctx->innatesEnabled && IsInnateActive(ctx->battlerAtk, ABILITY_SUPER_LUCK))) // FORK: innate Super Luck (see CalcCritChanceStage)
        critChance *= 4;
    ...
    // cached ctx->innatesEnabled gate short-circuits the IsInnateActive() lookups when the feature is off.
    else if (ctx->innatesEnabled
          && (IsInnateActive(ctx->battlerDef, ABILITY_BATTLE_ARMOR) || IsInnateActive(ctx->battlerDef, ABILITY_SHELL_ARMOR)))
    {
        critChance = CRITICAL_HIT_BLOCKED;
          ...
          || MoveAlwaysCrits(ctx->move)
          || ((ctx->abilities[ctx->battlerAtk] == ABILITY_MERCILESS
            || (ctx->innatesEnabled && IsInnateActive(ctx->battlerAtk, ABILITY_MERCILESS))) // FORK: innate Merciless (see CalcCritChanceStage)
              && gBattleMons[ctx->battlerDef].status1 & STATUS1_PSN_ANY))
    {
        critChance = CRITICAL_HIT_ALWAYS;
    }
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
        ...
        && !(ctx->holdEffects[ctx->battlerDef] == HOLD_EFFECT_RING_TARGET && IS_BATTLER_OF_TYPE(ctx->battlerDef, TYPE_FLYING) && !IsBattlerUngroundedByAbilityItemOrEffect(ctx->battlerDef, ctx->abilities[ctx->battlerDef], ctx->holdEffects[ctx->battlerDef])))
    {
        ...
        if (ctx->updateFlags && (ctx->abilities[ctx->battlerDef] == ABILITY_LEVITATE || ctx->abilities[ctx->battlerDef] == ABILITY_EELEVATE
         || IsInnateActive(ctx->battlerDef, ABILITY_LEVITATE))) // FORK: credit innate Levitate too
        {
            ...
            if (ctx->abilities[ctx->battlerDef] != ABILITY_LEVITATE && ctx->abilities[ctx->battlerDef] != ABILITY_EELEVATE)
            {
    ...
    // On conflict, port upstream's Telepathy change into telepathyDodge above; don't re-inline it here.
    bool32 telepathyDodge = ctx->battlerDef == GetPartnerBattler(ctx->battlerAtk)
...
uq4_12_t CalcPartyMonTypeEffectivenessMultiplier(enum Move move, enum Species speciesDef, enum Ability abilityDef)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        // FORK: innate-aware (species-level prediction, no battle state; config-gated so the feature
        // being off neither credits an innate nor pays for the override scan).
        if (ctx.moveType == TYPE_GROUND
         && (abilityDef == ABILITY_LEVITATE || abilityDef == ABILITY_EELEVATE
          || (GetConfig(FEATURE_INNATE_ABILITIES) && SpeciesHasInnate(speciesDef, ABILITY_LEVITATE)))
...
u32 CalcSecondaryEffectChance(enum BattlerId battler, enum Ability battlerAbility, const struct AdditionalEffect *additionalEffect)
{
    bool8 hasSereneGrace = (battlerAbility == ABILITY_SERENE_GRACE)
                        || IsInnateActive(battler, ABILITY_SERENE_GRACE); // FORK: innate-aware
...
bool32 CanMoveSkipAccuracyCalc(struct BattleCalcValues *cv, u32 weather, enum ResultOption option)
{
    ...
    {
        u32 attackerWeather = GetAttackerWeather(battlerAtk, cv->holdEffects[battlerAtk], cv->abilities[battlerAtk], weather);
...
u32 GetTotalAccuracy(struct BattleCalcValues *cv, u32 weather)
{
    ...
    // stays byte-for-byte untouched; the cheap category/threshold guards run before the IsInnateActive
    // lookup (this is the AI-hot accuracy calc). IsInnateActive supplies suppression parity (breakable).
    else if (moveAcc > 50 && IsBattleMoveStatus(cv->move)
    ...
    if (cv->abilities[battlerDef] != ABILITY_SAND_VEIL && (attackerWeather & B_WEATHER_SANDSTORM)
     && IsInnateActive(battlerDef, ABILITY_SAND_VEIL))
        calc = (calc * 80) / 100;
    if (cv->abilities[battlerDef] != ABILITY_SNOW_CLOAK && (attackerWeather & B_WEATHER_ICY_ANY)
    ...
    if (cv->abilities[battlerDef] != ABILITY_TANGLED_FEET && gBattleMons[battlerDef].volatiles.confusionTimer
    ...
    if (gBattleStruct->battlerState[battlerAtk].usedMicleBerry)
    {
        ...
        // Upstream dropped its own Ripen branch here in the 1.17.0 sync; the fork keeps the interaction.
        if (cv->abilities[battlerAtk] == ABILITY_RIPEN || IsInnateActive(battlerAtk, ABILITY_RIPEN))
