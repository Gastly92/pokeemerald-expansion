...
static bool32 ShouldSwitchIfIntimidateBenefit(struct SwitchAiContext *switchContext)
{
    ...
    if (IsBattlerAlive(switchContext->opposingBattler))
    {
        ...
        // FORK: a foe's innate Defiant / Competitive / Rattled turns our Intimidate into a boon for it
        // (FEATURE_INNATE_ABILITIES: +2 Atk / +2 Sp.Atk / +1 Speed), so don't switch an Intimidator in
        // expecting to weaken it — mirroring how DoesIntimidateRaiseStats already flags a chosen Rattled.
...
static u32 GetSwitchinSingleUseItemHealing(enum BattlerId battler, enum BattlerId opposingBattler, s32 currentHP)
{
    ...
    if (currentHP <= 0
     ...
     || ((gAiLogicData->abilities[opposingBattler] == ABILITY_UNNERVE
       || IsInnateActive(opposingBattler, ABILITY_UNNERVE)) // FORK: innate-aware Unnerve (FEATURE_INNATE_ABILITIES)
      && GetItemPocket(aiItem) == POCKET_BERRIES))
        return itemHeal;
...
static s32 GetSwitchinWeatherImpact(enum BattlerId battler)
{
     ...
     && !IsInnateActive(battler, ABILITY_OVERCOAT)) // FORK: innate Overcoat ignores sandstorm/hail chip too
    {
         ...
         && !IsInnateActive(battler, ABILITY_ICE_BODY)) // FORK: innate Ice Body ignores hail too
        {
            ...
            && !IsInnateActive(battler, ABILITY_SAND_FORCE)) // FORK: innate Sand Force ignores sandstorm too
        {
            weatherImpact = maxHP / 16;
            if (weatherImpact == 0)
                weatherImpact = 1;
    ...
    if ((status != 0) && ability != ABILITY_MAGIC_GUARD)
    {
        if (status & STATUS1_BURN)
        {
            ...
            if (ability == ABILITY_HEATPROOF
             || (GetConfig(FEATURE_INNATE_ABILITIES) && BattlerHasAbility(battler, ABILITY_HEATPROOF))) // FORK: innate Heatproof halves burn damage too
