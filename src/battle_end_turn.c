...
static bool32 HandleEndTurnWeatherDamage(enum BattlerId battler)
{
    ...
    case BATTLE_WEATHER_SANDSTORM:
        if (ability != ABILITY_SAND_VEIL
         ...
         && !IsAbilityOrInnateAndRecord(battler, ability, ABILITY_MAGIC_GUARD))
        {
    ...
    case FIRST_EVENT_BLOCK_GMAX_MOVE_RESIDUAL: // TODO: Has to be split into 3 statuses and needs a queue
        ...
        if (gSideStatuses[side] & SIDE_STATUS_DAMAGE_NON_TYPES)
        {
            if (!IS_BATTLER_OF_TYPE(battler, gSideTimers[side].damageNonTypesType)
             && !IsAbilityOrInnateAndRecord(battler, GetBattlerAbility(battler), ABILITY_MAGIC_GUARD))
            {
...
static bool32 HandleEndTurnLeechSeed(enum BattlerId battler)
{
    ...
    if (!GetConfig(BUFF_LEECH_SEED))
    {
        ...
        if (IsBattlerPresent(drainedBattler)
         ...
         && !IsAbilityOrInnateAndRecord(drainedBattler, GetBattlerAbility(drainedBattler), ABILITY_MAGIC_GUARD)) // FORK: innate-aware Magic Guard
        {
            ...
            if (BattlerHasAbility(drainedBattler, ABILITY_LIQUID_OOZE)) // FORK: innate-aware Liquid Ooze (FEATURE_INNATE_ABILITIES)
            {
...
static bool32 HandleEndTurnPoison(enum BattlerId battler)
{
    ...
    {
        if (IsAbilityOrInnateAndRecord(battler, ability, ABILITY_MAGIC_GUARD))
        {
