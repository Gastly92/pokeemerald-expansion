...
static bool32 DoesInnateBenefitFromTerrain(enum BattlerId battler, enum BattleTerrain terrain); // FORK: FEATURE_INNATE_ABILITIES
...
// FORK: FEATURE_INNATE_ABILITIES — innate-aware companion to DoesAbilityBenefitFromTerrain (which
// stays upstream-pristine). TRUE if the battler has an *active innate* terrain-keyed ability (Surge Surfer
// on Electric Terrain, Grass Pelt on Grassy Terrain) that benefits from `terrain`, so the AI's
// terrain-setting heuristics value setting it. ORed in at each DoesAbilityBenefitFromTerrain call
...
static bool32 DoesInnateBenefitFromTerrain(enum BattlerId battler, enum BattleTerrain terrain)
{
    return (terrain == B_TERRAIN_ELECTRIC && IsInnateActive(battler, ABILITY_SURGE_SURFER))
        || (terrain == B_TERRAIN_GRASSY   && IsInnateActive(battler, ABILITY_GRASS_PELT));
}
...
static enum FieldEffectOutcome BenefitsFromSandstorm(enum BattlerId battler)
{
    ...
    if (gAiLogicData->holdEffects[battler] == HOLD_EFFECT_SAFETY_GOGGLES || IS_BATTLER_ANY_TYPE(battler, TYPE_ROCK, TYPE_GROUND, TYPE_STEEL))
    {
        if (!IS_BATTLER_ANY_TYPE(GetBattlerLeftFoe(battler), TYPE_ROCK, TYPE_GROUND, TYPE_STEEL)
         ...
         && !DoesInnateBenefitFromWeather(GetBattlerLeftFoe(battler), B_WEATHER_SANDSTORM)) // FORK: innate-aware
            return FIELD_EFFECT_POSITIVE;
        else
            return FIELD_EFFECT_NEUTRAL;
    }
...
static enum FieldEffectOutcome BenefitsFromElectricTerrain(enum BattlerId battler)
{
    if (DoesAbilityBenefitFromTerrain(gAiLogicData->abilities[battler], B_TERRAIN_ELECTRIC)
     || DoesInnateBenefitFromTerrain(battler, B_TERRAIN_ELECTRIC)) // FORK: innate-aware
        return FIELD_EFFECT_POSITIVE;
...
static enum FieldEffectOutcome BenefitsFromGrassyTerrain(enum BattlerId battler)
{
    if (DoesAbilityBenefitFromTerrain(gAiLogicData->abilities[battler], B_TERRAIN_GRASSY)
     || DoesInnateBenefitFromTerrain(battler, B_TERRAIN_GRASSY)) // FORK: innate-aware
        return FIELD_EFFECT_POSITIVE;
...
static enum FieldEffectOutcome BenefitsFromPsychicTerrain(enum BattlerId battler)
{
    ...
    if (grounded || allyGrounded)
    {
        ...
        if (AI_IsAbilityOnSide(GetBattlerLeftFoe(battler), ABILITY_GALE_WINGS)
         || AI_IsInnateOnSide(GetBattlerLeftFoe(battler), ABILITY_GALE_WINGS) // FORK: innate-aware
         || AI_IsAbilityOnSide(GetBattlerLeftFoe(battler), ABILITY_TRIAGE)
         || AI_IsInnateOnSide(GetBattlerLeftFoe(battler), ABILITY_TRIAGE) // FORK: innate-aware
         || AI_IsAbilityOnSide(GetBattlerLeftFoe(battler), ABILITY_PRANKSTER)
         || AI_IsInnateOnSide(GetBattlerLeftFoe(battler), ABILITY_PRANKSTER)) // FORK: innate-aware
            return FIELD_EFFECT_POSITIVE;
    }
    ...
    if (AI_IsAbilityOnSide(battler, ABILITY_GALE_WINGS)
     || AI_IsInnateOnSide(battler, ABILITY_GALE_WINGS) // FORK: innate-aware
     || AI_IsAbilityOnSide(battler, ABILITY_TRIAGE)
     || AI_IsInnateOnSide(battler, ABILITY_TRIAGE) // FORK: innate-aware
