...
bool32 Ai_IsPriorityBlocked(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    // FORK: replaces upstream's IsDazzlingAbility(aiData->abilities[...]) — GetBattlerDazzlingAbility()
    // also finds an innate blocker. On conflict, keep this call, not IsDazzlingAbility().
    if (IsDoubleBattle()
     && GetBattlerDazzlingAbility(GetPartnerBattler(battlerDef), aiData->abilities[GetPartnerBattler(battlerDef)]) != ABILITY_NONE)
        return TRUE;
...
bool32 IsAdditionalEffectBlocked(enum BattlerId battlerAtk, enum Ability abilityAtk, enum BattlerId battlerDef, enum Ability abilityDef, enum Move move)
{
    ...
    if ((abilityDef == ABILITY_SHIELD_DUST || IsInnateActive(battlerDef, ABILITY_SHIELD_DUST)) // FORK: innate Shield Dust blocks additional effects too
     && !IsMoldBreakerTypeAbility(battlerAtk, abilityAtk, move))
        return TRUE;
...
bool32 CanEndureHit(enum BattlerId battler, enum BattlerId battlerTarget, enum Move move)
{
    ...
    {
        if (GetConfig(B_STURDY) >= GEN_5 && BattlerHasAbility(battlerTarget, ABILITY_STURDY)) // FORK: innate-aware
            return TRUE;
...
bool32 ShouldTryOHKO(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability atkAbility, enum Ability defAbility, enum Move move)
{
    ...
    if (!DoesBattlerIgnoreAbilityChecks(battlerAtk, atkAbility, move) && BattlerHasAbility(battlerDef, ABILITY_STURDY)) // FORK: innate-aware
        return FALSE;
...
bool32 IsTwoTurnNotSemiInvulnerableMove(enum BattlerId battlerAtk, enum Move move)
{
    ...
    case EFFECT_TWO_TURNS_ATTACK:
    {
        ...
        u32 attackerWeather = GetAttackerWeather(battlerAtk, gAiLogicData->holdEffects[battlerAtk], gAiLogicData->abilities[battlerAtk], weather);
...
bool32 BattlerHasMaxHPProtection(enum BattlerId battler)
{
    ...
    if (GetConfig(B_STURDY) >= GEN_5 && ability == ABILITY_STURDY)
        return TRUE;
    if (GetConfig(B_STURDY) >= GEN_5 && IsInnateActive(battler, ABILITY_STURDY)) // FORK: innate-aware Sturdy
        return TRUE;
...
bool32 AI_CanPutToSleep(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability defAbility, enum Move move, enum Move partnerMove)
{
    if (!CanBeSlept(battlerAtk, battlerDef, defAbility, BLOCKED_BY_SLEEP_CLAUSE)
      ...
      || StatusWillBeCuredDeterministically(battlerDef, defAbility) // FORK: DETERMINISTIC_STATUS
...
{
    enum Move predictedMove = GetPredictedMove(battlerAtk, battlerDef, gAiLogicData);
    if (((!IsMoldBreakerTypeAbility(battlerAtk, gAiLogicData->abilities[battlerAtk], move)
...
bool32 ShouldRestoreHpBerry(enum BattlerId battlerAtk, enum Item item)
{
    ...
    case ITEM_ORAN_BERRY:
        ...
        // meaningful on a tiny HP pool. BUFF_FLAT_HP_ITEMS makes it maxHP/4 at every level, so
...
s32 AI_GetAdjustedStatStage(enum BattlerId battler, enum Move move, s32 stage)
{
    if (GetMoveEffect(move) == EFFECT_GROWTH
     && GetAttackerWeather(battler, gAiLogicData->holdEffects[battler], gAiLogicData->abilities[battler], AI_GetWeather()) & B_WEATHER_SUN)
        stage = 2;
...
static bool32 AI_CanStatChangeBePrevented(struct BattleCalcValues *cv)
{
    ...
    default:
        if (IsSubstituteProtected(cv->battlerAtk, cv->battlerDef, cv->abilities[cv->battlerAtk], cv->move)
         || BattlerHasAbility(cv->battlerDef, ABILITY_GOOD_AS_GOLD)) // FORK: credit an innate Good as Gold (blocks the stat-lowering status move)
            return TRUE;
    }
