...
#define UNPACK_FEATURE_CONFIG_CHANGES(_name, _field, ...) ._field = _name, // FORK
...
const struct ConfigChanges sConfigChanges =
{
    ...
    FEATURE_CONFIG_DEFINITIONS(UNPACK_FEATURE_CONFIG_CHANGES) // FORK
    /* Expands to:
...
ARM_FUNC u32 GetConfigInternal(enum ConfigTag _config)
{
    ...
    if (gConfigChangesTestOverride == NULL)
    {
        ...
        FEATURE_CONFIG_DEFINITIONS(UNPACK_CONFIG_GETTERS) // FORK
        /* Expands to:
...
#endif
    {
        ...
        FEATURE_CONFIG_DEFINITIONS(UNPACK_CONFIG_OVERRIDE_GETTERS) // FORK
        /* Expands to:
        case CONFIG_CRIT_CHANCE:
...
u32 GetClampedValue(enum ConfigTag _config, u32 newValue)
{
    ...
    FEATURE_CONFIG_DEFINITIONS(UNPACK_CONFIG_CLAMPER) // FORK
    default:
        return 0;
    }
    return clampedValue;
...
void SetConfig(enum ConfigTag _config, u32 _value)
{
    ...
    FEATURE_CONFIG_DEFINITIONS(UNPACK_CONFIG_SETTERS) // FORK
    /* Expands to:
    #if TESTING
    case CONFIG_CRIT_CHANCE:
...
void TestInitConfigData(void)
{
    ...
    FEATURE_CONFIG_DEFINITIONS(UNPACK_FORK_FLAG_FORCE_OFF) // FORK
}
