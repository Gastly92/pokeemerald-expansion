...
#define B_CLEAN_HEALTHBOX                   TRUE       // FORK: If set to TRUE, declutters the battle healthboxes: removes the triangular pointer/tail on each box's corner (it points at the mon and, in doubles, can cover the on-healthbox type icons), and removes the EXP bar + its wrap-around frame from the singles player box.
...
#define B_USE_FROSTBITE                 TRUE       // FORK: upstream default is FALSE. In PLA, Frostbite replaces Freeze. Enabling this flag does the same here. Moves can still be cherry-picked to either Freeze or Frostbite. Freeze-Dry, Secret Power & Tri Attack depend on this config.
...
#define B_SHOW_TYPES        SHOW_TYPES_ALWAYS    // FORK: upstream default is SHOW_TYPES_NEVER. When to show type indicators next to Pokémon HP bars, while choosing a move after selecting a target Pokémon.
...
#define B_SHOW_EFFECTIVENESS        SHOW_EFFECTIVENESS_ALWAYS // FORK: upstream default is SHOW_EFFECTIVENESS_SEEN. If not SHOW_EFFECTIVENESS_NEVER, the PP string is replaced by a type effectiveness indicator based on the move and the target.
