...
#define START_WITH_BATTLE_GIMMICK_ITEMS TRUE  // If TRUE, a new game gives the player the four battle-transformation key items (Mega Ring, Z-Power Ring, Dynamax Band, Tera Orb) in the bag. Mega Evolution, Z-Moves and Ultra Burst work immediately (item is all they need). Dynamax and Tera additionally consult B_FLAG_DYNAMAX_BATTLE / B_FLAG_TERA_ORB_* (config/battle.h) -- but ONLY when FEATURE_FREE_GIMMICKS is off, and only for the player's side: CanDynamax() and CanTerastallize() skip the item AND flag gates entirely under that feature (and AI battlers skip them unconditionally). Since FEATURE_FREE_GIMMICKS defaults TRUE, all four gimmicks are live for both sides and these key items are a convenience for a free-gimmicks-off build. See GiveStartingBattleGimmickItems() in src/new_game.c.
