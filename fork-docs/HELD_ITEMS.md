...
| ~~**Blunder Policy**~~ — **shipped** | Rebuilt, now **2 sets** | `DETERMINISTIC_HOLD_EFFECTS` re-armed it on the deterministic blunders (Protect, semi-invulnerability, Wide/Quick/Crafty Guard, Psychic Terrain, a type immunity, a blocking ability, an Air Balloon). In doubles any one avoiding target arms it. Placed on two sets whose **own STAB carries a hard immunity**, so the trigger is a matter of course rather than luck: Diggersby (Return and Quick Attack blanked by Ghost, Drill Run by Flying/Levitate — and it is a Swords Dance set at 78 Speed, so the blundered turn becomes the sweep window) and Brambleghast (Poltergeist and Shadow Sneak blanked by Normal, and its Choice Band was locking a set that carries Rapid Spin). Note the doubles spread-move idea does **not** apply here: the roster deliberately runs no Earthquake on a doubles set, since it hits the ally. |
...
Combat, and Arceus-Ground runs a Cosmic Power / Earthquake stall build, all collecting
...
- **A defensive item must be able to fire at all.** Before placing a type-conditional item,
  ...
  in the first resist-berry batch, when both still ran Well-Baked Body. That immunity has since
  been taken off them — Well-Baked Body erased the 4x Fire weakness that is their only real
  check, so the override rows now read Bulletproof (Scizor, Mega Scizor) and Toxic Debris
  (Ferrothorn) — which reopens the Occa Berry for both, so they are the first place to look in
  the next berry batch. The abilities
