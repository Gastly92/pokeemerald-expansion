...
| [`HELD_ITEMS.md`](HELD_ITEMS.md) | Held-item usage across the roster, and the `BUFF_*` backlog for the unused ones (status is tracked in `test/fork/held_item_tracker.c`) |
...
| In-battle INFO viewer | `B_FRONTIER_BATTLE_INFO` | `config/frontier.h`, `src/fork/frontier_battle_info.c` | ⚠️ partial | In facilities where the bag is disabled, the BAG slot (and SELECT in the battle party menu) opens a read-only six-page reference: Speed Tiers, Field, Conditions, Stat Changes, Foe, Innates. Strictly reveal-gated and Illusion-safe; stock assets only. Details: [`BATTLE_INFO.md`](BATTLE_INFO.md). Foe page reads opponent A only. |
