...
| Deterministic additional effects | `DETERMINISTIC_ADDITIONAL_EFFECTS` | ✅ | A move's sub-100% secondary effect lands on a fixed condition instead of a roll: a super-effective hit (types that *can* be SE) or a STAB user (Normal). Serene Grace / Pledge Rainbow make it *certain*, and G-Max Replenish's 50% berry recovery always lands. [Mechanics](DETERMINISM.md#deterministic_additional_effects) · `test/fork/deterministic_additional_effects.c` |
