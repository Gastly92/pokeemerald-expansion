...
| `DETERMINISTIC_STATUS` | Sleep is a fixed `DETERMINISTIC_SLEEP_TURNS` (3) — but the wake-up turn is still an acting turn, so N costs the target **N−1** actions. Rest is exempt. Confusion = one 80-BP self-hit (`BUFF_CONFUSION_SELF_DAMAGE`) on the battler's next action of any kind, which still happens; it then lingers and snaps out on the action after. Infatuation lasts 2 actions and halves damage instead of blocking it. |
