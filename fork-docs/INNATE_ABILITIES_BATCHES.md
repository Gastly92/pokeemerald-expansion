...
| W10 | Batch J end-of-turn all-innate (**split by dex**) | Blastoise, Dewgong, Chansey, Blissey, Ludicolo, Manaphy, Phione, Darkrai, the Avalugg / Exeggutor / Bellossom / Walrein / Luvdisc / Gorebyss / Whiscash / Tropius / Glaceon / Gliscor lines, … | ~40 | open (chunk **W10a done**: dex ≤ 272 — Blastoise, Dewgong, Exeggutor + Alola, Bellossom freed via fork chosen overrides Blastoise→Water Absorb (empty slot 1), Dewgong→Snow Warning (slot-2 Ice Body; no test uses Dewgong), Exeggutor + Exeggutor-Alola→Sap Sipper (empty slot 1), Bellossom→Effect Spore (empty slot 1). **Chansey & Blissey excluded** — extremely test-heavy standard bulky mons: Blissey is the chosen-differs-from-innate exemplar for TWO innates in `test/fork/innate_abilities.c` (Serene Grace vs Natural Cure, Natural Cure vs Healer) and Chansey has slot-0 Natural Cure default-critical + slot-2 Healer pinned by `healer.c` / `sleep_clause.c`, so both keep chosen Healer (redundant-but-correct), like Snorlax/Clefable. **Ludicolo excluded** — all three slots pinned (Swift Swim in `swift_swim.c`, Rain Dish in `rain_dish.c` / `neutralizing_gas.c`, Own Tempo the chosen-differs exemplar in `innate_abilities.c`), keeps chosen Rain Dish. Remaining W10b+ (Manaphy / Phione / Darkrai / Walrein / Luvdisc / Gorebyss / Whiscash / Tropius / Glaceon / Gliscor / Trevenant / Avalugg-Hisui / Swanna / Cyclizar lines) still open) |
