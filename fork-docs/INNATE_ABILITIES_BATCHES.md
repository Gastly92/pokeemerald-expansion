...
| W9 | Batch I status-immunity all-innate (**split by dex**) | ~24 sets — see the Batch I note in `FORK.md`; re-grep the status-immunity innates | ~24 | open (chunk **W9a done**: dex ≤ 235 — Slowbro-Galar, Hitmonchan, Crobat, Slowking, Smeargle freed; fork chosen overrides Slowbro-Galar→Poison Touch (slot 2), Hitmonchan→No Guard (slot 2), Slowking→Water Absorb (slot 2), Crobat→Reckless (existing slot-1 override, both remaining Inner Focus sets repointed), and free-real-slot repoint Smeargle→Moody (real slot 2, :x:). **Slowbro, Hypno & Kangaskhan excluded** — a slot is the chosen-differs-from-innate exemplar in `test/fork/innate_abilities.c` (Slowbro→Regenerator, Hypno→Inner Focus, Kangaskhan→Inner Focus) and/or all other slots are test-pinned/default-critical, so they keep chosen Own Tempo / Inner Focus (redundant-but-correct), like Clefable/Snorlax. Chunk **W9b done**: dex 321–549 — Wailord, Camerupt, Grumpig, Glalie, Lucario, Leafeon, Throh, Leavanny, Lilligant, Lilligant-Hisui freed; fork chosen overrides Wailord→Water Absorb (slot 2), Camerupt→Flash Fire (slot 2), Grumpig→Synchronize (slot 1), Leafeon→Sap Sipper (duplicate slot 1), Throh→Simple (slot 1), Leavanny→Sharpness (slot 2), Lilligant→Grassy Surge (slot 2), and free-real-slot repoints Glalie→Moody (real slot 2), Lucario→Steadfast (real slot 0), Lilligant-Hisui→Hustle (real slot 1). **Meowstic excluded** — its chosen Own Tempo is an implemented innate it does NOT carry (stable, like Carnivine), so not a deferral. Remaining dex > 560 open — Escavalier, Mienshao, Mandibuzz, Avalugg, Tsareena, Dondozo) |
