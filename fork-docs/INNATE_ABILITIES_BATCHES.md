...
| W6 | Batch K steal / Liquid Ooze / Cursed Body all-innate | Tentacruel, Swalot, Weavile, Grimmsnarl, Delphox, Klefki, Hoopa (+Unbound), Froslass, Banette | 21 | done (fork chosen overrides: Tentacruel→Water Absorb (slot 2), Swalot→Poison Touch (slot 2), Banette→Wandering Spirit (slot 2), Weavile→Tough Claws (empty slot 1), Froslass→Snow Warning (empty slot 1), Delphox→Flash Fire (empty slot 1), Klefki→Bulletproof (empty slot 1), Hoopa→Tinted Lens (empty slot 1), Hoopa-Unbound→Tough Claws (empty slot 1), Grimmsnarl→Infiltrator (slot 2); all `.ability` rows repointed. Every species had all real abilities innate; empty-slot fills where a slot was ABILITY_NONE, else an unpinned innate-redundant real slot repurposed after auditing test/ pins) |
| W7 | Batch L Download all-innate | Porygon-Z, Genesect | 4 | done (fork chosen overrides: Porygon-Z→Simple (slot-2 Analytic HA repurposed; slot-0 Adaptability test-pinned so kept), Genesect→Sheer Force (empty slot 1); both `.ability` rows repointed off the now-innate Download) |
| W8 | Batch L Intimidate all-innate (**split by dex**) | the ~40 sets that hardcoded Intimidate | ~40 | done (chunk **W8a done**: dex ≤ 237 — Arbok, Arcanine + Hisui, Tauros + Paldea trio, Gyarados, Granbull, Qwilfish, Hitmontop; fork chosen overrides Arbok→Poison Point, Tauros-Paldea→Sheer Force, Gyarados→Motor Drive, Granbull→Static, Hitmontop→No Guard, and free-real-slot repoints Arcanine/Hisui→Flash Fire, Tauros→Sheer Force, Qwilfish→Poison Point. Chunk **W8b done**: dex 262–560 — Mightyena, Masquerain, Mawile, Salamence, Staraptor, Luxray, Stoutland, Krookodile, Scrafty; fork chosen overrides Mightyena→Sheer Force (slot 2), Masquerain→Storm Drain (empty slot 1), Salamence→Rivalry (empty slot 1), Staraptor→Hustle (empty slot 1), Stoutland→Sheer Force (slot 2), Krookodile→Sand Stream (slot 2), Scrafty→Rivalry (slot 1), and free-real-slot repoints Mawile→Sheer Force (slot 2), Luxray→Rivalry (slot 0). Chunk **W8c done**: dex ≥ 727 — Incineroar (empty slot-1 override → Tough Claws), Mabosstiff (existing slot-2 Strong Jaw override, remaining Intimidate set repointed), and free-real-slot repoints Wyrdeer→Sap Sipper (real slot 2), Overqwil→Poison Point (real slot 0), Squawkabilly-Green→Hustle (real slot 1). No `.ability = ABILITY_INTIMIDATE` frontier row remains) |
| W9 | Batch I status-immunity all-innate (**split by dex**) | ~24 sets — see the Batch I note in `FORK.md`; re-grep the status-immunity innates | ~24 | done (chunk **W9a done**: dex ≤ 235 — Slowbro-Galar, Hitmonchan, Crobat, Slowking, Smeargle freed; fork chosen overrides Slowbro-Galar→Poison Touch (slot 2), Hitmonchan→No Guard (slot 2), Slowking→Water Absorb (slot 2), Crobat→Reckless (existing slot-1 override, both remaining Inner Focus sets repointed), and free-real-slot repoint Smeargle→Moody (real slot 2, :x:). **Slowbro, Hypno & Kangaskhan excluded** — a slot is the chosen-differs-from-innate exemplar in `test/fork/innate_abilities.c` (Slowbro→Regenerator, Hypno→Inner Focus, Kangaskhan→Inner Focus) and/or all other slots are test-pinned/default-critical, so they keep chosen Own Tempo / Inner Focus (redundant-but-correct), like Clefable/Snorlax. Chunk **W9b done**: dex 321–549 — Grumpig, Glalie, Lucario, Leafeon, Throh, Leavanny, Lilligant, Lilligant-Hisui freed; fork chosen overrides Grumpig→Synchronize (slot 1), Leafeon→Sap Sipper (duplicate slot 1), Throh→Simple (slot 1), Leavanny→Sharpness (slot 2), Lilligant→Grassy Surge (slot 2), and free-real-slot repoints Glalie→Moody (real slot 2), Lucario→Steadfast (real slot 0), Lilligant-Hisui→Hustle (real slot 1). **Wailord & Camerupt excluded** — both are used in AI tests sensitive to the whole ability SET (Wailord in ai.c's "best OHKO move" — a move-absorbing chosen ability makes the AI dodge Water Spout; Camerupt in ai_thinking_time.c, whose node-count ceiling an extra immunity branch tips over), so they keep chosen Water Veil / Magma Armor (redundant-but-correct), like Slowbro/Snorlax. **Meowstic excluded** — its chosen Own Tempo is an implemented innate it does NOT carry (stable, like Carnivine), so not a deferral. Chunk **W9c done** (completes W9): dex > 560 — Escavalier, Mienshao, Mandibuzz, Avalugg, Tsareena freed; fork chosen overrides Escavalier→Sheer Force (slot 2), Mienshao→No Guard (slot 2), Mandibuzz→Unaware (slot-2 Weak Armor drawback), Avalugg→Ice Scales (slot 2; both sets, one had run the now-innate Ice Body), Tsareena→Grassy Surge (slot-2 Sweet Veil; non-immunity pick to avoid perturbing its ai.c/dazzling.c priority-block tests). **Dondozo excluded** — slots 1/2 (Oblivious/Water Veil) are both chosen-differs exemplars and slot-0 Unaware is the default read in AI tests (ai_check_viability/ai_switching), so it keeps chosen Water Veil (redundant-but-correct), like Slowbro/Snorlax) |
| W10 | Batch J end-of-turn all-innate (**done in one pass**) | Blastoise, Dewgong, Chansey, Blissey, Ludicolo, Manaphy, Phione, Darkrai, the Avalugg / Exeggutor / Bellossom / Walrein / Luvdisc / Gorebyss / Whiscash / Tropius / Glaceon / Gliscor lines, … | ~40 | done (**completed in a single PR** rather than the planned dex chunks). Fork chosen overrides: Blastoise→Water Absorb, Gorebyss/Luvdisc/Phione/Manaphy→Water Absorb, Walrein→Water Absorb (empty-slot fills except Walrein slot-1 Ice Body), Dewgong/Glaceon→Snow Warning (slot-2/slot-2 Ice Body; self-synergistic snow), Exeggutor + Exeggutor-Alola→Sap Sipper (empty slot 1), Bellossom→Effect Spore (empty slot 1), Whiscash/Swanna→Storm Drain (slot-2 Hydration), Darkrai→Sheer Force (empty slot 1), Trevenant→Sap Sipper (slot-2 Harvest), Avalugg-Hisui→Ice Scales (slot-1 Ice Body, matching base Avalugg), Cyclizar→Motor Drive (empty slot 1). **Tropius** repointed to its real slot-1 Solar Power (`:x:`, no override; ai_doubles.c already relies on it). **Excluded** (kept redundant-but-correct chosen ability, like Snorlax/Clefable): **Chansey & Blissey** (extremely test-heavy standard bulky mons — Blissey is the chosen-differs-from-innate exemplar for TWO innates in `test/fork/innate_abilities.c`, Chansey has slot-0 Natural Cure default-critical + slot-2 Healer pinned by `healer.c` / `sleep_clause.c`), **Ludicolo** (all three slots pinned: `swift_swim.c` / `rain_dish.c` / `neutralizing_gas.c` / the Own Tempo chosen-differs exemplar), and **Gliscor** (all three slots pinned: slot-0 Hyper Cutter the `ai_switching.c` default read, slot-1 Sand Veil the chosen-differs exemplar in `innate_abilities.c`, slot-2 Poison Heal pinned by `ai_doubles.c`). |
...
| ⟂ | Batch W — Frontier-slot freeing sweep (Step 3.5 backlog, I/J/T/K/L/U + Tier 5/Y tails) | data cleanup, multi-session | done (all of the ordered [W1–W10 sub-queue](#batch-w-sub-queue-the-breakdown) is complete). **The entire execution queue and Batch W now read `done`, so per this doc's lifecycle rule it is eligible for retirement** — the progress doc + [wiring reference](INNATE_ABILITIES.md) are the complete record. |
...
| W — Frontier-slot freeing sweep | data cleanup, multi-session | ~24 I-sets + ~40 J-sets + T/K/L/U + Tier 5.4–5.9/Y5 tails | done (ordered [W1–W10 breakdown](#batch-w-sub-queue-the-breakdown) all complete) |
