...
| Frontier max PP | `B_FRONTIER_MAX_PP` | `config/frontier.h` | ⚠️ partial | Maxes PP Up bonus on all four move slots in `CreateFacilityMon`, gated on `FLAG_FRONTIER_MON_FACTORY`. Covers the whole Battle Factory roster (rentals, opponents, Frontier Brain). Other facilities don't set the flag yet — they follow in later passes. |
