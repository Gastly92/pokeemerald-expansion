...
| In-battle INFO viewer | `B_FRONTIER_BATTLE_INFO` | `config/frontier.h` | ⚠️ partial | In facilities where the bag is disabled (`BATTLE_TYPE_FRONTIER_NO_PYRAMID` — Tower, Dome, Palace, Arena, Factory, Pike; **not** the Pyramid, which keeps a working bag), the BAG action slot becomes **INFO**, opening a read-only screen (`src/frontier_battle_info.c`, `CB2_FrontierBattleInfo`). Three pages, R/L cycle: **Field** (weather/terrain + entry hazards & side screens both sides), **Conditions** (each on-field battler's primary status + notable volatiles — confusion, leech seed, taunt, etc. — both sides), **Foe** (the foe's **revealed-only** party data; `<>` cycles mons). "Seen" comes from `gBattleStruct->partyState[...].sentOut`; species/level read from the party once seen; moves/PP/ability/held-item shown only once revealed in battle (`gAiPartyData[B_SIDE_OPPONENT]`). Reuses the `B_ACTION_DEBUG` controller path so the turn isn't consumed; touches only the player controller (`FORK:` tags) + a menu-label string. **Foe page reads opponent A only** (single-trainer doubles / singles); two-opponent multi battles show just the first foe trainer's party for now. |
