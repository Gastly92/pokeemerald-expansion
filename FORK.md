...
| Deterministic flinch | `DETERMINISTIC_FLINCH` | `config/deterministic.h` | ✅ | A move's flinch additional effect stops being a random roll and lands every time the move connects, **except** when the target was flinched on the *previous* turn — so flinch is reliable but a fast flincher can't chain it into an inescapable flinch-lock (each flinch "uses up" the next turn's). Implemented in `TryTriggerAdditionalEffect()` (`src/battle_util.c`); the "flinched last turn" bit is snapshotted per battler in `HandleEndTurn_ContinueBattle` (`src/battle_main.c`, `BattlerState.flinchedLastTurn`) just before the volatile is cleared. Guaranteed flinches (chance ≥ 100%) and **Fake Out** / any first-turn-only flincher are exempt (a first-turn move can't be used on consecutive turns, so it always flinches). Inner Focus / Shield Dust / Covert Cloak immunity unchanged. The AI's flinch valuation (`IsFlinchGuaranteed`, `AI_CalcAdditionalEffectScore` via `AI_IsAdditionalEffectReliable`) is taught the rule. Required by `DETERMINISTIC_ADDITIONAL_EFFECTS`, which delegates all flinch effects here. **Enabled (`TRUE`).** Covered by `test/battle/deterministic_flinch.c`. |
| Deterministic additional effects | `DETERMINISTIC_ADDITIONAL_EFFECTS` | `config/deterministic.h` | ✅ | A move's chance-based additional effect (burn, paralysis, a stat drop, …) stops being a random roll and lands on a fixed, state-based condition: if the move's type **can** be super effective (every type but Normal in the stock chart), the effect lands only on a super effective hit (e.g. Fire Punch burns only a Fire-weak target); if the type **never** can (Normal), it lands only when the move is **STAB** (e.g. Body Slam paralyzes only from a Normal user). **Only applies to sub-100% effects** — guaranteed (≥ 100%) or chance-less (primary) effects always land, unchanged. The stock secondary-chance boosters (**Serene Grace** and the **Pledge Rainbow**), which would otherwise be near-useless under this flag, instead make the holder's effects *certain* — they bypass the super-effective/STAB gate and always land (detected as "computed chance > base chance" so both sources and their quirks fall out for free). **Flinch is excluded** and handled by `DETERMINISTIC_FLINCH` instead (this flag requires it); the boosters do **not** lift flinch's anti-lock rule, so they can't restore flinch-lock. Implemented in `TryTriggerAdditionalEffect()` / `DeterministicAdditionalEffectApplies()` (`src/battle_util.c`), called from `Cmd_setadditionaleffects`; the AI is taught the same condition via `AI_IsAdditionalEffectReliable` (`src/battle_ai_util.c`, used by `AI_CalcAdditionalEffectScore`). **Enabled (`TRUE`).** Covered by `test/battle/deterministic_additional_effects.c`. (The super-effective branch relies on a recorded damage result; in practice every one of the game's chance-based additional effects is on a damaging move, so this is always available.) |
...
`DETERMINISTIC_DAMAGE`, `DETERMINISTIC_PARALYSIS`, `DETERMINISTIC_FLINCH`, and
`DETERMINISTIC_ADDITIONAL_EFFECTS` (all enabled — see table). The general shape
